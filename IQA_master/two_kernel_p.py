...
model_urls = {
    ...
    'densenet161': 'https://download.pytorch.org/mode ls/densenet161-8d451a50.pth',
}
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.k2 = nn.Parameter(torch.tensor(0.3333, dtype=torch.float32))
    ...
    def forward(self, x):
        new_features = super(_DenseLayer, self).forward(self.k2*x)
        if self.drop_rate > 0:
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24),
        ...
        # First convolution
        #self.k2 = nn.Parameter(torch.tensor(0.333, dtype=torch.float32))
    ...
    def forward(self, x):
        ...
        x = self.features.denseblock1(x) #256,56,56
        x_56 = x
        #print('5', x.shape)
...
'''
feature = torch.rand(10, 3, 224, 224)

net = densenetnew(pretrained=False)
state_dict2 = net.features.state_dict()
        
for name in list(state_dict2.keys()):
    print(name)
