...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.k3 = nn.Parameter(torch.tensor(0.3333, dtype=torch.float32))
        self.add_module('norm1', nn.BatchNorm2d(num_input_features)),
        self.add_module('relu1', nn.ReLU(inplace=True)),
        self.add_module('conv1', nn.Conv2d(num_input_features, bn_size *
                                           growth_rate, kernel_size=5, stride=1, 
    ...
    def forward(self, x):
        new_features = super(_DenseLayer, self).forward(self.k3*x)
        if self.drop_rate > 0:
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24),
        ...
        # First convolution
        #self.k3 = nn.Parameter(torch.tensor(0.333, dtype=torch.float32))
